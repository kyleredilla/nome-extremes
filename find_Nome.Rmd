...
lsm_path <- "F:/raw_data/Nome_Mets/ERA5/ERA5_land_sea_mask_Nome_sector.nc"
...
The bounds for the nine ERA5 cells around Nome (N and S and the two adjacent inland (E)) are: `r kable(bb)` and are saved as **Nome_bb.csv**  

# WRF

Display Nome in the WRF grid

```{r}
# extract lon and lat from nc file and close
fn <- "F:/raw_data/Nome_Mets/geo_em.d01.nc"
lsm_nc <- nc_open(lsm_path)
lon <- ncvar_get(lsm_nc, varid = "longitude")
lat <- ncvar_get(lsm_nc, varid = "latitude")
lsm <- ncvar_get(lsm_nc, varid = "lsm")
